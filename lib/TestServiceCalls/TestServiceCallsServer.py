...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'TestServiceCalls'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from TestServiceCalls.TestServiceCallsImpl import TestServiceCalls
impl_TestServiceCalls = TestServiceCalls(config)
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'TestServiceCalls'
        ...
        self.rpc_service.add(impl_TestServiceCalls.get_feature_ids,
                             name='TestServiceCalls.get_feature_ids',
                             types=[dict])
        self.method_authentication['TestServiceCalls.get_feature_ids'] = 'required'
        self.rpc_service.add(impl_TestServiceCalls.status,
                             name='TestServiceCalls.status',
                             types=[dict])
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "TestServiceCalls but no authentication header was passed"
                            raise err
